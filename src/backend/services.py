...
#     except Exception as e:
#         # 실제 오류를 터미널에 출력
#         print(f"🚨 T2I Core Inference Error: {e}") 
#         import traceback
#         traceback.print_exc() # ⬅️ 상세한 트레이스백 출력
#         raise # 상위 호출자(main.py)에게 다시 예외 발생
...
# services.py
# ============================================================
# SDXL + GPT-5 Mini 추론 로직 (독립 모듈)
# - 모델 초기화 함수(init_sdxl_pipelines)
# - 캡션 생성(generate_caption_core)
# - T2I/I2I 동기 생성 함수(generate_t2i_core / generate_i2i_core)
# ============================================================
...
# # Globals
# openai_client: Optional[OpenAI] = None
# T2I_PIPE: Optional[StableDiffusionXLPipeline] = None
# I2I_PIPE: Optional[StableDiffusionXLImg2ImgPipeline] = None
# DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

# # Initialize OpenAI client (attempt)
# if OPENAI_API_KEY:
#     try:
#         openai_client = OpenAI(api_key=OPENAI_API_KEY)
#     except Exception as e:
...
# # ===========================
# def init_sdxl_pipelines():
#     """SDXL T2I / I2I 파이프라인을 로드합니다. FileLock으로 중복 로드 방지."""
#     global T2I_PIPE, I2I_PIPE, DEVICE

#     lock_path = os.path.join(hf_cache_dir, "sdxl_init.lock")
#     lock = FileLock(lock_path, timeout=600)  # 10분 타임아웃

#     try:
#         with lock:
#             if T2I_PIPE is not None and I2I_PIPE is not None:
#                 print("ℹ️ SDXL 파이프라인 이미 로드됨 — 스킵")
#                 return

#             print(f"📦 SDXL 모델 로딩 시작 (Device={DEVICE}, cache={hf_cache_dir})")
#             dtype = torch.float16 if DEVICE == "cuda" else torch.float32
...
#                 print("✅ SDXL 파이프라인 로딩 성공")
#             except Exception as e:
#                 print(f"❌ SDXL 로딩 실패: {e}")
#                 print(traceback.format_exc())
#                 # GPU OOM 시 CPU로 폴백 시도
...
#         print(f"⚠️ SDXL 초기화 락 에러: {lock_e}")
#         print(traceback.format_exc())
...
#             max_output_tokens=512,
#         )
#         # OpenAI Python client 형태 따라 안전하게 텍스트 추출
...
# # ===========================
# # SDXL 동기 추론 함수 (run_in_executor로 호출되도록 설계)
# # ===========================
# def generate_t2i_core(prompt: str, width: int, height: int, steps: int) -> bytes:
#     global T2I_PIPE
#     if T2I_PIPE is None:
#         raise RuntimeError("SDXL T2I 파이프라인이 초기화되지 않았습니다.")
#     negative_prompt = "low quality, blurry, text, watermark, distorted, ugly, tiling, poorly drawn"
#     result = T2I_PIPE(
#         prompt=prompt,
#         negative_prompt=negative_prompt,
...
#     global I2I_PIPE
#     if I2I_PIPE is None:
#         raise RuntimeError("SDXL I2I 파이프라인이 초기화되지 않았습니다.")
#     negative_prompt = "low quality, blurry, text, watermark, distorted, ugly, tiling, poorly drawn"
#     input_image = Image.open(io.BytesIO(input_image_bytes)).convert("RGB").resize((width, height))
#     result = I2I_PIPE(
#         prompt=prompt,
