...
3. FLUX 모델을 위한 3단계 프롬프트 파이프라인 적용 (T2I/I2I 공용)
...
규칙: 장면의 배경, 조명, 동작, 분위기, 구도 등을 자연스럽고 구체적으로 확장합니다.
...
"""
    ...
    try:
        ...
        print(f"🔄 1/3 확장 (한국어): {expanded[:80]}...")
        return expanded
    except Exception as e:
...
# =======================================================
# 2단계: FLUX 템플릿 기반 영어 변환
# =======================================================
def apply_flux_template(expanded_kor_text: str) -> str:
    """확장된 한국어 설명을 FLUX 스타일 영어 프롬프트로 변환"""
    if not openai_client:
...
"""
    ...
    try:
        ...
        print(f"🔄 2/3 템플릿 (영어): {templated[:80]}...")
        return templated
    except Exception as e:
...
# =======================================================
# 3단계: 최종 최적화 (FLUX/SDXL 공용 폴리셔)
# =======================================================
def optimize_prompt(text: str, model_config) -> str:
    """FLUX/SDXL 최종 프롬프트 다듬기"""
    if not openai_client:
        return text
...
"""
            ...
            constraint = (
                f"Keep it under 60 words (model has {max_tokens} token limit)."
                if max_tokens <= 77
                else "Keep it concise but descriptive (under 150 words)."
            )
...
"""
        ...
        print(f"🔄 3/3 최종 최적화: {optimized[:80]}...")
        return optimized
...
# =======================================================
# 공용: 최종 프롬프트 빌더 (T2I / I2I 둘 다 사용)
# =======================================================
def build_final_prompt(raw_prompt: str) -> str:
    """
    프론트엔드에서 넘어온 raw_prompt(한국어/영어/혼합)를
...
def apply_adetailer(
    ...
    """ADetailer 스타일 후처리: 손/얼굴 감지 후 해당 영역만 Inpaint"""
    global model_loader
    ...
    try:
        ...
        if info.get("processed"):
            ...
            print("ℹ️ ADetailer: 보정 대상 없음, 원본 유지")
...
def generate_t2i_core(
    ...
    """텍스트 기반 이미지 생성 (FLUX 3단계 파이프라인 적용)"""
    global model_loader
    ...
    max_steps = getattr(model_config, "max_steps", steps) if model_config else steps
    steps = ensure_steps(min(steps, max_steps))
    ...
    # Negative Prompt (모델별 기본값)
    if "flux" in model_type:
        gen_params["negative_prompt"] = "no distortions, no extra limbs, missing fingers, watermark, low quality"
    elif getattr(model_config, "use_negative_prompt", False):
        gen_params["negative_prompt"] = getattr(model_config, "negative_prompt", "")

    # Guidance Scale
    if guidance_scale is not None:
        gen_params["guidance_scale"] = guidance_scale
    elif getattr(model_config, "guidance_scale", None) is not None:
        gen_params["guidance_scale"] = model_config.guidance_scale

    print(f"🎨 T2I 생성 중 (Model: {model_loader.current_model_name}, Prompt: {final_prompt[:80]}...)")
...
def generate_i2i_core(
    ...
    """이미지 기반 편집 (I2I - FLUX 3단계 파이프라인 동일 적용)"""
    global model_loader
    ...
    if not getattr(model_config, "supports_i2i", False):
