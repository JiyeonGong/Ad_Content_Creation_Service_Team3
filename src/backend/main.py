...
# @app.on_event("startup")
# async def startup_event():
#     print("🔥 FastAPI Startup: SDXL 모델 초기화 시작")
#     services.init_sdxl_pipelines()
#     print("🔥 FastAPI Startup: SDXL 모델 초기화 완료")
...
# # main.py
# # ============================================================
# # FastAPI 게이트웨이 - services.py의 함수 사용
# # ============================================================
...
# app = FastAPI(title="헬스케어 AI 콘텐츠 API (서비스 분리형)")

# # Pydantic schemas (간결화)
# class CaptionRequest(BaseModel):
#     service_type: str
#     service_name: str
#     features: str
...
# # Startup event: SDXL을 1회 로드 (services 내부의 init 호출)
# @app.on_event("startup")
# async def startup_event():
#     loop = asyncio.get_event_loop()
#     # services.init_sdxl_pipelines는 블로킹이므로 executor에서 실행
#     await loop.run_in_executor(None, services.init_sdxl_pipelines)
...
#     # 안전 상한 (메모리 보호)
#     if width > 2048 or height > 2048:
#         raise HTTPException(status_code=400, detail="width/height 값이 너무 큽니다. 2048 이하로 설정하세요.")
...
#         err = str(e).lower()
#         if "out of memory" in err or "cuda" in err:
#             raise HTTPException(status_code=503, detail="GPU 메모리 부족 또는 CUDA 문제 발생")
#         raise HTTPException(status_code=500, detail=f"T2I 생성 실패: {e}")
