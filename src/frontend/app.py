...
#         # 선택 박스의 포맷 함수는 버전 번호만 표시하도록 변경
#         idx = st.selectbox("이미지 선택", range(len(preloaded)), format_func=lambda x: f"버전 {x+1}", key="i2i_image_selector")
        
#         # BytesIO 객체에서 바이트 읽기
#         image_bytes = get_image_bytes(preloaded[idx]["bytes"])
#         display_image = image_bytes
...
#                 with col2:
#                     st.subheader("편집됨")
#                     st.image(edited_bytes, use_container_width=True)
